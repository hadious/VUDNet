        ...
        for i, (images_test, depth_maps_test, identifiers) in enumerate(test_loader):
            ...
            if save_images:
                 for j, identifier in enumerate(identifiers):  # Iterate through batch
                    ...
                    file_name = f'PDVAE{identifier}.npy'
                    file_path = os.path.join("VAE/VAE_DEPTH", file_name)
...
def Main():
    ...
    # test_loss = 0.0
    # with torch.no_grad():
    #     for images_test, depth_maps_test in data_loader:
    ...
    metrics = test_model(model, data_loader, device)
    print(metrics)
Main()
