...
GAN = False
VAE_DEPTH = False
INPUT_CHANNELS = "Three"


class Surface_VH_with_VAE(Dataset):
    ...
    def __getitem__(self, index):
        ...
        npy_file_name = f'PDVAE{os.path.splitext(image_name)[0]}.png.npy'  # Construct .npy filename
        npy_file_path = os.path.join("VAE/VAE_DEPTH", npy_file_name)
        Vae_depth = np.load(npy_file_path)  # Load the .npy file
        ...
        if VAE_DEPTH == False:
            ...
            if INPUT_CHANNELS == "Nine":
                ...
                wpx_wpy_img = np.stack((img), axis=0)
            elif INPUT_CHANNELS == "Three":
                wpx_wpy_img = np.stack((wpx, wpy, img), axis=0)
        else:
            
            if INPUT_CHANNELS == "Nine":
                wpx_wpy_img = np.stack((wpx, wpy, img, magnitude_img, direction_img, magnitude_wpx, direction_wpx, magnitude_wpy, direction_wpy, Vae_depth), axis=0)
            elif INPUT_CHANNELS == "One":
                wpx_wpy_img = np.stack((img, Vae_depth), axis=0)
            elif INPUT_CHANNELS == "Three":
                wpx_wpy_img = np.stack((wpx, wpy, img, Vae_depth), axis=0)
            # wpx_wpy_img = np.stack((wpx, wpy, img, magnitude_img, direction_img, magnitude_wpx, direction_wpx, magnitude_wpy, direction_wpy, Vae_depth), axis=0)
            # import pdb
            # pdb.set_trace()
