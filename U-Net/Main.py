...
#loss?
#Idea one (Phase-Based Depth Loss): phase difference between pattern and the image (or the parts of image!!!) can be a good reward or punishment! more freq? deepr??? shallower? what?
    # then we can calculate Fourier and have it as a criteria

# Idea Two (Gradient Density): if edges (sobel) are nearer or farther
# Idea Three: Fringe Contrast Depth Loss, chnage in intensity 
